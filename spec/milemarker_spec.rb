...
RSpec.describe Milemarker do
  ...
  it "triggers on on_batch" do
    triggers = []
    mm = basic_mm(5, 12) { |i, mm| triggers << i }
    expect(triggers).to contain_exactly(5, 10)
  end

  it "time marches forward" do
    times = []
    mm = basic_mm(5, 12) { |i, mm| times << mm.total_seconds_so_far}
  ...
  it "outputs batch log lines as expected" do
    ...
    expect { (1..12).each {|i| mm.increment_and_log_batch_line}}.to output(r).to_stdout_from_any_process
  end
