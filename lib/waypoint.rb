...
class Waypoint
  ...
  def batch_rate_str(decimals = 0)
    ...
    ppnum(last_batch_size.to_f / last_batch_seconds)
    # format "%5.#{decimals}f",
  end
  ...
  def total_rate_str(decimals = 0)
    ...
    ppnum(count / total_seconds_so_far)
  end

  def batch_line
    # rubocop:disable Layout/LineLength
    "#{file_or_process} #{ppnum(count, 10)}. This batch #{ppnum(last_batch_size, 5)} in #{ppnum(last_batch_seconds, 4, 1)}s (#{batch_rate_str} r/s). Overall #{total_rate_str} r/s."
    # rubocop:enable Layout/LineLength
  end

  def final_line
    # rubocop:disable Layout/LineLength
    "#{file_or_process} FINISHED. #{ppnum(count, 10)} total records in #{seconds_to_time_string(total_seconds_so_far)}. Overall #{total_rate_str} r/s."
    # rubocop:enable Layout/LineLength
  end
